...
public class TokenStreamRewriteEngine implements TokenStream {
	...
	public void rollback(String programName, int instructionIndex) {
		...
		if ( is!=null ) {
			programs.put(programName, is.subList(0,instructionIndex));
		}
	}
	...
	protected void addToSortedRewriteList(String programName, RewriteOperation op) {
		...
		// if last op's index, just append
	...
	public void insertAfter(String programName, Token t, String text) {
		...
		addToSortedRewriteList(programName,
							   new InsertBeforeOp(((TokenWithIndex)t).getIndex()+1,text));
	}
	...
	public void insertBefore(String programName, Token t, String text) {
		addToSortedRewriteList(new InsertBeforeOp(((TokenWithIndex)t).getIndex(),text));
	}
	...
	public void replace(String programName, Token from, Token to, String text) {
		addToSortedRewriteList(
				new ReplaceOp(((TokenWithIndex)from).getIndex(),
							  ((TokenWithIndex)to).getIndex(), text));
	}
	...
	public String toOriginalString() {
		return toOriginalString(0, getTokenStreamSize());
	}
	...
	public String toString(String programName, int start, int end) {
		...
		int i=start;
		while ( i>=0 && i<=end && i<tokens.size() ) {
			if ( rewriteOpIndex<rewrites.size() ) {
				RewriteOperation op =
						(RewriteOperation)rewrites.get(rewriteOpIndex);
				boolean moreOpsAtThisIndex = true;
				while ( i==op.index && rewriteOpIndex<rewrites.size() ) {
					i = op.execute(buf);
					rewriteOpIndex++;
					if ( rewriteOpIndex<rewrites.size() ) {
